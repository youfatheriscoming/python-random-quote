#!/usr/bin/env python3
"""Build smile quotes (ATM, RR, BF) from SPX option market quotes.

Input format is a CSV file with columns:
...
- delta_label: absolute target delta bucket (e.g. 0.50, 0.25, 0.10)

The script computes Black-Scholes implied vols and prints, for each expiry:
- ATM volatility (using the 50-delta quotes, averaged between call/put if both exist)
...
def main() -> None:
    ...
    parser.add_argument("--quotes", required=True, help="CSV path with quote rows")
    parser.add_argument("--spot", type=float, required=True, help="Spot price")
    parser.add_argument("--rate", type=float, required=True, help="Risk-free annualized rate, decimal")
    parser.add_argument("--div", type=float, required=True, help="Dividend yield / carry q, decimal")
    parser.add_argument(
        "--valuation-date",
